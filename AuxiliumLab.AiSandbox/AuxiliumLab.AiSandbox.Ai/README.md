...
This project is the **AI decision-making and training orchestration layer** of the AuxiliumLab sandbox. It defines how agents choose actions at each simulation step — whether through random selection, through a trained Stable-Baselines3 (SB3) reinforcement-learning model running in Python, or through deterministic inference with a pre-trained model. It also contains the gRPC client for communicating with the Python RL service, training algorithm wrappers, and environment specification negotiation.
...
├── InferenceActions.cs         # Inference-only: drives pre-trained model via gRPC Act RPC
├── ObservationBuilder.cs       # Shared observation encoding + action decoding
├── BaseTraining.cs             # Abstract base for training algorithm settings
├── EnvironmentSpecBuilder.cs   # Builds and validates EnvironmentSpec for Python negotiation
├── Configuration/
...
│   ├── PolicyTrainerClientServiceCollectionExtensions.cs  # gRPC client DI
│   ├── RewardSettings.cs       # Step/win/loss reward values
│   ├── TrainingAlgorithmSettings.cs  # Per-algorithm config
│   ├── TrainingParameter.cs    # Name/Value pair for hyperparameters
│   └── TrainingSettings.cs     # Top-level training config
├── GrpcClients/
│   ├── IPolicyTrainerClient.cs # gRPC client interface
│   └── PolicyTrainerClient.cs  # gRPC client implementation
├── Trainers/
│   ├── ITraining.cs            # Training algorithm interface
│   ├── PpoTraining.cs          # PPO algorithm wrapper
│   ├── A2cTraining.cs          # A2C algorithm wrapper
│   └── DqnTraining.cs          # DQN algorithm wrapper
└── Protos/
    └── policy_trainer.proto    # gRPC service definition (client stubs generated by Grpc.Tools)
```
