...
class GrpcExternalEnvAdapter(ExternalEnvAdapter):
    ...
    Connection is established lazily on first use to avoid blocking during
    multi-env setup.
    """
    ...
    def __init__(self, grpc_endpoint: str, gym_id: str = ""):
        ...
        The gRPC channel is created lazily on the first reset() or step() call,
        not here, to avoid blocking the constructor (important when creating
        multiple adapters for DummyVecEnv).
        ...
        self._channel = None
        self._stub = None
        self._sim_pb2 = None  # Cached reference to simulation_pb2 module
    
    def _ensure_connected(self) -> None:
        """Establish gRPC channel on first use (lazy connect)."""
        if self._stub is not None:
            return

        try:
            from generated import simulation_pb2, simulation_pb2_grpc
            self._sim_pb2 = simulation_pb2
    ...
    def reset(self, seed: Optional[int] = None) -> np.ndarray:
        ...
        for attempt in range(max_retries):
            try:
                request = self._sim_pb2.ResetRequest(
                    gym_id=self.gym_id, seed=seed if seed is not None else 0
                )
                response = self._stub.Reset(request, timeout=60)
                ...
                raise RuntimeError(
                    f"Failed to reset .NET simulation at {self.grpc_endpoint} "
                    f"for gym_id={self.gym_id} after {max_retries} attempts: {e}"
                ) from e
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        ...
        try:
            request = self._sim_pb2.StepRequest(gym_id=self.gym_id, action=action)
            ...
            raise RuntimeError(
                f"Failed to step .NET simulation at {self.grpc_endpoint} "
                f"for gym_id={self.gym_id}, action={action}: {e}"
            ) from e
    
    def close(self) -> None:
        """Close gRPC channel."""
        try:
            if self._stub is not None and self._sim_pb2 is not None:
                request = self._sim_pb2.CloseRequest(gym_id=self.gym_id)
